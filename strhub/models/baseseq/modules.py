...
class Encoder(VisionTransformer):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, embed_dim=768, depth=12, num_heads=12, mlp_ratio=4.,
        ...
        super().__init__(img_size, patch_size, in_chans, embed_dim=embed_dim, depth=depth, num_heads=num_heads,
                         ...
                         num_classes=0, global_pool=global_pool, class_token=class_token)  # these disable the classifier head
...
class StageEncoder(VisionTransformer):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, embed_dim=768, depth=12, num_heads=12, mlp_ratio=4.,
        ...
        super().__init__(img_size, patch_size, in_chans, embed_dim=embed_dim, depth=depth, num_heads=num_heads,
                         ...
                         drop_path_rate=drop_path_rate, embed_layer=embed_layer,
                         num_classes=0, global_pool=global_pool, class_token=class_token)  # these disable the classifier head
        self.return_ind = [depth // 4, (depth // 4) * 2, (depth // 4) * 3, depth]
    ...
    def forward_features(self, x):
        ...
        if self.grad_checkpointing and not torch.jit.is_scripting():
            ...
            return_list[-1] = self.norm(return_list[-1])
            return return_list
...
class StageDecoder(nn.Module):
    ...
    def __init__(self, decoder_layer, num_layers, norm, return_idx=[0,1,2,3]):
        super().__init__()
        self.layers = transformer._get_clones(decoder_layer, num_layers)
        self.num_layers = num_layers
        self.norm = norm
    ...
    def forward(self, query, content, memory, query_mask: Optional[Tensor] = None, content_mask: Optional[Tensor] = None,
        ...
        for i, mod in enumerate(self.layers):
            ...
            if i in self.return_idx:
                return_list.append(self.norm(query))
        # query = self.norm(query)
        return return_list
