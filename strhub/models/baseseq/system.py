...
class DATR(BaseSeq):
    ...
    def similar_cosine_batch(self, images1: Tensor, images2: Tensor):
        with torch.no_grad():
            images1_clip = images1 * 0.5 + 0.5
            images1_clip = (images1_clip - self.clip_mean) / self.clip_std
            image1_features = self.clip.encode_image(images1_clip)
            norm_image1_features = image1_features / image1_features.norm(dim=1, keepdim=True)

            images2_clip = images2 * 0.5 + 0.5
            images2_clip = (images2_clip - self.clip_mean) / self.clip_std
            image2_features = self.clip.encode_image(images2_clip)
            norm_image2_features = image2_features / image2_features.norm(dim=1, keepdim=True)
            similarity_matrix = torch.mm(norm_image1_features, norm_image2_features.transpose(0, 1))
            _, indices = similarity_matrix.max(dim=1)
            similar_images = images2[indices]
    ...
    def training_step(self, batch, batch_idx) -> STEP_OUTPUT:
        ...
        images1 = self.preprocess_images(images[:half])
        labels1 = labels[:half]
        images2 = self.preprocess_images(images[half:])
        labels2 = labels[half:]
        similar_images, indices = self.similar_cosine_batch(images1, images2)
        images2 = similar_images
        indices_list = indices.tolist()
        matched_labels = [labels2[i] for i in indices_list]
        labels2 = matched_labels
        ...
        with torch.no_grad():
            ...
            tensor = primary_f_list_vis[-1]
            torch.set_printoptions(profile="full")
            tensor_str = str(tensor)
        
        # Compute similarity and apply sigmoid
        similarity = F.cosine_similarity(helper_clip_embed, primary_clip_embed, dim=-1)
        similarity_weight = torch.sigmoid(similarity)
        ...
        # Weighted loss
        distillation_loss = loss_word + loss_align + loss_align_vis
        WLCL = loss_char + (distillation_loss * similarity_weight)
